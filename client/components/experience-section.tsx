...
export function ExperienceSection() {
  return (
    <section id="experience" className="py-20 bg-background dark:bg-warm-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-8">
          {experiences.map((exp, index) => (
            <motion.div
              key={index}
              className="bg-warm-gray-50 dark:bg-warm-gray-700 rounded-xl p-8 hover:shadow-lg transition-all duration-300"
              initial={{ opacity: 0, y: 50 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
                <div>
                  ...
                  <p className={`text-xl font-semibold ${exp.color}`}>{exp.company}</p>
                  <p className="text-warm-gray-600 dark:text-warm-gray-300">{exp.location}</p>
                </div>
                <div className="text-warm-gray-500 dark:text-warm-gray-400 mt-2 lg:mt-0 flex items-center">
              ...
              <ul className="space-y-3 text-warm-gray-600 dark:text-warm-gray-400">
                {exp.achievements.map((achievement, achievementIndex) => (
                  <motion.li
                    ...
                    viewport={{ once: true }}
                  >
                    <ChevronRight className={`${exp.color} mr-3 mt-1 h-4 w-4 flex-shrink-0`} />
